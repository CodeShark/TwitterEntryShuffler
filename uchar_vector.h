...
public:
    ...
    const uchar_vector operator^(const std::vector<unsigned char>& rightOperand)
    {
        if (this->size() != rightOperand.size())
            throw std::runtime_error("Operands for uchar_vector::operator^ are not the same length.");

        uchar_vector result;
        for (int i = 0; i < rightOperand.size(); i++)
        {
            result.push_back((*this)[i] ^ rightOperand[i]);
        }

        return result;
    }

    bool operator<(const std::vector<unsigned char>& rightOperand)
    {
        int left_i = 0;
        int right_i = 0;

        for (; left_i < this->size(); left_i++)
        {
            if ((*this)[left_i] != 0) break;
        }

        for (; right_i < rightOperand.size(); right_i++)
        {
            if (rightOperand[right_i] != 0) break;
        }

        if ((this->size() - left_i) < (rightOperand.size() - right_i)) return true;
        if ((this->size() - left_i) > (rightOperand.size() - right_i)) return false;

        for (; left_i < this->size(); left_i++, right_i++)
        {
            if ((*this)[left_i] < rightOperand[right_i]) return true;
            if ((*this)[left_i] > rightOperand[right_i]) return false;
        }
